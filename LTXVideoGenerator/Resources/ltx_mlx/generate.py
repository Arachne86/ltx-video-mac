...
def denoise(
    ...
    for i in range(total_steps):
        ...
        # Output progress
        progress_output(
            stage, i + 1, total_steps, f"Denoising step {i+1}/{total_steps}"
        )
...
def generate_video(
    ...
    status_output(
        f"Starting {mode_str} generation: {width}x{height}, {num_frames} frames"
    )
    ...
    vae_decoder = load_vae_decoder(
        str(model_path / "ltx-2-19b-distilled.safetensors"), timestep_conditioning=None
    )

    latents = upsample_latents(
        latents, upsampler, vae_decoder.latents_mean, vae_decoder.latents_std
    )
    mx.eval(latents)
    ...
    if tiling_config is not None:
        video = vae_decoder.decode_tiled(
            latents, tiling_config=tiling_config, tiling_mode=tiling
        )
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output-path", type=str, default="output.mp4", help="Output path"
    )
    parser.add_argument(
        "--model-repo",
        type=str,
    ...
    parser.add_argument(
        "--image", "-i", type=str, default=None, help="Conditioning image"
    )
    parser.add_argument(
        "--image-strength", type=float, default=1.0, help="Image conditioning strength"
    )
    parser.add_argument(
        "--tiling",
        type=str,
