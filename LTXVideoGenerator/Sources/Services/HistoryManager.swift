...
class HistoryManager: ObservableObject {
    ...
    private let ioQueue = DispatchQueue(label: "com.ltxvideo.history.io", qos: .background)
    ...
    // ⚡ Bolt Optimization: Offload synchronous disk I/O to a background queue
    // Impact: Eliminates main thread blocking when saving history, preventing UI hitches
    ...
    func deleteResult(_ result: GenerationResult) {
        // ⚡ Bolt Optimization: Offload synchronous file deletion to background queue
        // Impact: Prevents UI hitching during file cleanup
        let videoURLToDelete = result.videoURL
        let thumbnailURLToDelete = result.thumbnailURL
        
        ioQueue.async {
            try? FileManager.default.removeItem(at: videoURLToDelete)
            if let thumb = thumbnailURLToDelete {
    ...
    func deleteResults(_ resultsToDelete: Set<GenerationResult>) {
        // ⚡ Bolt Optimization: Offload synchronous file deletions to background queue
        // Impact: Prevents UI hitching during batch file cleanup
        let urlsToDelete = resultsToDelete.map { ($0.videoURL, $0.thumbnailURL) }

        ioQueue.async {
            for (videoURL, thumbnailURL) in urlsToDelete {
    ...
    func clearHistory() {
        // ⚡ Bolt Optimization: Offload synchronous file deletions to background queue
        // Impact: Prevents UI hitching when clearing entire history
        let urlsToDelete = results.map { ($0.videoURL, $0.thumbnailURL) }
