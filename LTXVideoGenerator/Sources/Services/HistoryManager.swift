...
class HistoryManager: ObservableObject {
    ...
    private func saveHistory() {
        ...
        // Impact: Eliminates main thread blocking and prevents UI hitches
    ...
    func deleteResults(_ resultsToDelete: Set<GenerationResult>) {
        ...
        // ⚡ Bolt Optimization: Offload synchronous file deletion to background queue
        let urlsToDelete = resultsToDelete.map { (videoURL: $0.videoURL, thumbnailURL: $0.thumbnailURL) }

        ioQueue.async {
            for urls in urlsToDelete {
                try? FileManager.default.removeItem(at: urls.videoURL)
                if let thumbURL = urls.thumbnailURL {
                    try? FileManager.default.removeItem(at: thumbURL)
                }
            }
        }
    ...
    func clearHistory() {
        // ⚡ Bolt Optimization: Offload synchronous file deletion to background queue
        let urlsToDelete = results.map { (videoURL: $0.videoURL, thumbnailURL: $0.thumbnailURL) }
