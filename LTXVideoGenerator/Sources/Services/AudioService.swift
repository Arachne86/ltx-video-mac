...
enum MusicGenre: String, CaseIterable, Identifiable {
    ...
    // ⚡ Bolt Optimization: Replace computed property with lazily-initialized static let to avoid re-evaluating
    // the static array filtering on every UI render pass, especially when accessed by ForEach loops.
    // Impact: Eliminates redundant O(N) filtering operations per access.
