...
struct HistoryThumbnailView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            // ⚡ Bolt Optimization: Offload synchronous disk I/O to avoid blocking main thread
            // Impact: Eliminates main thread blocking and stuttering when scrolling history view
            .task(id: result.id) {
                loadedImage = nil
                guard let thumbnailURL = result.thumbnailURL else { return }

                let image = await Task.detached(priority: .background) {
                    NSImage(contentsOf: thumbnailURL)
                }.value
